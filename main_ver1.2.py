...
if __name__=="__main__":
    ...
    while True:
        ...
        path = table.find('g',class_='highcharts-series highcharts-series-0 highcharts-line-series ')
        ...
        #꺾은선
        print("\t>>> 꺾은선 추출 시작")
        texts = bs4.find('g',class_='highcharts-axis-labels highcharts-xaxis-labels ').find_all('text')
        datadic = []
        for text in texts:
            datadic.append((text.get_text()+'desktop','-1'))
            datadic.append((text.get_text() + 'mobile', '-1'))
        datadic = dict(datadic)
        ...
        try:
            for test in driver.find_elements_by_tag_name('rect')[9:16]:
                try:
                    tmp = driver.find_element_by_tag_name('path')
                    hovertmp = ActionChains(driver).move_to_element(tmp)
                    hovertmp.perform()
                    ...
                    time.sleep(0.5)
                    bs4 = BeautifulSoup(driver.page_source, "lxml")
                    div = bs4.find_all('div',class_='highcharts-label highcharts-tooltip highcharts-color-undefined')[-1]
                    key, data = str(div.get_text()).split(':')
        ...
        idx = 0
        try:
            for test in driver.find_elements_by_tag_name('rect')[20:]:
                try:
                    hover = ActionChains(driver).move_to_element(test)
                    hover.perform()
                    tmp = driver.find_element_by_tag_name('path')
